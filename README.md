...
- 🧩 **コードブロック単体保存**: ChatGPT / Claudeのコードブロックから、そのブロックだけをMarkdownとして保存
...
| **コードブロック内の保存ボタン** | 押したコードブロックだけを保存 | コード、プロンプト断片、箇条書き素材だけを別ノート化 |
...
ChatGPT、Claude、Geminiでは、コードブロック内にも保存ボタンを表示します。コードブロック内に表示される保存ボタンを押した場合だけ、オプションの**コードブロックの保存先**を使います。会話全体保存の中に含まれるコードブロックを自動で別ファイル化する仕様ではありません。

Claudeでは、既存の「クリップボードにコピー」ボタンと同じhover/focus表示領域にObsidian保存ボタンを挿入します。通常時はClaudeのコピー操作と同じく非表示で、コードブロックへマウスを乗せるかフォーカスしたときに表示されます。

Geminiでは、コードブロック上部の操作列にObsidian保存ボタンを挿入します。既存のダウンロードbuttonとコピーbuttonの間に常時表示され、押したコードブロックだけをMarkdown fenced blockとして保存します。
...
src/
├── contentScripts/
│   ├── js/inject.js              # 対応チャットページに注入される共通処理
│   ├── js/providers/             # ChatGPT / Claude / Gemini別のDOM抽出・ボタン配置
│   └── css/inject.css            # インライン保存ボタン用CSS
├── chromium/
│   ├── background/               # 保存処理、Obsidian URI、Downloads fallback
│   ├── popup/                    # ポップアップUI
│   └── options/                  # オプションUI
├── utils/
│   ├── chat/                     # 保存先、テンプレート、フォーマット関連
│   └── markdown.js               # HTML→Markdown変換
└── __tests__/                    # provider契約・DOM抽出テスト
```
...
- オプションの「チャットページ上の保存ボタン表示」が有効になっているか確認してください

**Q: Claudeのコードブロック保存ボタンが見えない**
- Claudeのコードブロックでは、既存のコピーbuttonと同じくhover/focus時だけ表示されます
- コードブロック右上のコピーbuttonが出る位置にマウスを移動してください
- 表示されない場合はページを再読み込みし、最新の `dist-chromium` を読み込み直してください

**Q: Geminiのコードブロック保存ボタンが見えない**
- Geminiのコードブロック上部にあるダウンロードbuttonとコピーbuttonの間を確認してください
- 表示されない場合はページを再読み込みし、最新の `dist-chromium` を読み込み直してください
